...
data = sc.textFile("%s/results/parameters/female/2016031518_100" % (HDFS_OUTPUT_PATH))

data.map(lambda x:json.loads(x)).map(lambda x:(int(x[1][0]), x[1][1])).map(getThreshold)\
    .filter(getInvalid).count()

with open('/home/hadoop/chen.cheng/Chronos/parameters/AUC_2016031518_100', 'w') as f:
